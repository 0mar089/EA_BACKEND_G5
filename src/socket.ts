  ...
  io.on('connection', (socket) => {
    ...
    socket.on(
      ...
      async ({ destinatarioId, contenido, postId, parentMessageId, isGroup }) => {
        Logging.info(
          `[Socket] Mensaje recibido de ${userId} para ${destinatarioId} | isGroup: ${isGroup} | postId: ${postId} | parent: ${parentMessageId}`,
        );
        ...
        if (group && sender) {
          for (const memberId of group.miembros) {
            ...
            if (memberIdStr !== userId) {
              io.to(`user_${memberIdStr}`).emit('user_typing', {
                userId,
      ...
      if (isGroup) {
        ...
        if (group) {
          for (const memberId of group.miembros) {
            const memberIdStr = memberId.toString();
            if (memberIdStr !== userId) {
              io.to(`user_${memberIdStr}`).emit('user_stop_typing', {
                userId,
                grupoId: destinatarioId,
              });
            }
      ...
      try {
        ...
        if (isGroup) {
          ...
          if (group) {
            for (const memberId of group.miembros) {
              const memberIdStr = memberId.toString();
              io.to(`user_${memberIdStr}`).emit('messages_deleted', {
                messageIds,
                type,
                grupoId: destinatarioId,
              });
          ...
          if (type === 'everyone' && destinatarioId) {
            io.to(`user_${destinatarioId}`).emit('messages_deleted', {
              messageIds,
              type: 'everyone',
            });
          }
