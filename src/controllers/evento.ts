...
const asistirEvento = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    Logging.error(
      `[500] [evento] Attend Failed | eventoId=${eventoId}: ${(error as Error).message}`,
    );
    return res.status(500).json({ message: 'Internal server error' });
  }
};
...
const deleteEvento = async (req: AuthRequest, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if ((error as Error).message === 'Forbidden') {
      Logging.warning(
        `[403] [evento] Forbidden Delete | eventoId=${eventoId} userId=${req.user.id}`,
      );
      return res.status(403).json({ message: 'No tienes permiso para eliminar este evento' });
    }
    Logging.error(
      `[500] [evento] Delete Failed | eventoId=${eventoId}: ${(error as Error).message}`,
    );
    return res.status(500).json({ message: 'Internal server error' });
  }
};
