...
import { getMutualFollows, getConversation, markAsRead, getUnreadCount, getMessageById, getConversationForAdmin, createGroup, getGroupsForUser, getGroupConversation, markGroupAsRead } from '../services/chat';
...
export const getMessage = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (!hasAccess && message.grupo) {
            ...
            if (group) {
                hasAccess = group.miembros.map((m: mongoose.Types.ObjectId) => m.toString()).includes(requesterId);
            }
        }
...
export const getContacts = async (req: AuthRequest, res: Response) => {
    try {
        ...
        Logging.info(`[200] [chat] Contacts & Groups Retrieved | userId=${userId} contactsCount=${contacts?.length ?? 0} groupsCount=${groups?.length ?? 0}`);
...
export const createGroupChat = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (!miembros || !Array.isArray(miembros)) {
            return res.status(400).json({ message: 'Los miembros del grupo son obligatorios y deben ser una lista' });
        }
...
export const getHistory = async (req: AuthRequest, res: Response) => {
    try {
        ...
        if (group) {
            const isMember = group.miembros.map((m: mongoose.Types.ObjectId) => m.toString()).includes(myId);
            if (!isMember) {
                Logging.warning(`[403] [chat] Unauthorized Group Access | userId=${myId} groupId=${userId}`);
        ...
        const isContact = contacts.some(
            (c: { _id: unknown }) => String(c._id) === userId
        );
