...
const getUsuarioBasic = async (usuarioId: string): Promise<IUsuarioModel | null> => {
  return await Usuario.findOne({ _id: usuarioId, activo: true })
    .select('nombre universidad avatarUrl descripcion email privado')
...
const softDeleteUsuario = async (usuarioId: string): Promise<IUsuarioModel | null> => {
  ...
  return await Usuario.findByIdAndUpdate(usuarioId, { activo: false }, { new: true });
};
...
const recoveryUsuario = async (usuarioId: string): Promise<IUsuarioModel | null> => {
  ...
  return await Usuario.findByIdAndUpdate(usuarioId, { activo: true }, { new: true });
};
...
const hardDeleteUsuario = async (usuarioId: string): Promise<IUsuarioModel | null> => {
  ...
  const likesCleanup = await Post.updateMany({ likes: usuarioId }, { $pull: { likes: usuarioId } });
  ...
  await Usuario.updateMany({ seguidos: usuarioId }, { $pull: { seguidos: usuarioId } });
  // Quitar al usuario de la lista de 'seguidores' de otros (el usuario les seguía)
  await Usuario.updateMany({ seguidores: usuarioId }, { $pull: { seguidores: usuarioId } });
