...
const updateBugReportStatus = async (bugId: string, estado: string): Promise<IBugReportModel | null> => {
    return await BugReport.findByIdAndUpdate(bugId, { estado }, { new: true }).populate('usuarioReporta', 'nombre email avatarUrl');
};
