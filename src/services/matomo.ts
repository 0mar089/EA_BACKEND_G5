...
class MatomoService {
        ...
        try {
            ...
            .then(async (response) => {
                if (!response.ok) {
                    ...
                    Logging.warning(`[Matomo] Server returned status ${response.status} ${response.statusText} - Body: ${errorText}`);
                } else {
    ...
    public trackEvent(req: Request, category: string, action: string, name?: string, value?: number): void {
        const { ip, userAgent, lang, userId, fullUrl } = this.getClientDetails(req);
