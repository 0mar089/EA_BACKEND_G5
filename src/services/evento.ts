...
const createEvento = async (data: any, creadorId: string): Promise<IEventoModel | null> => {
    const { titulo, descripcion, fecha, ubicacionNombre, lat, lng, maxAsistentes } = data;
...
const asistirEvento = async (eventoId: string, userId: string): Promise<IEventoModel | null> => {
    ...
    if (yaAsiste) {
        // Abandonar evento
        evento.asistentes = evento.asistentes.filter((asistenteId) => asistenteId.toString() !== userId);
    } else {
        // Apuntarse a evento
...
const deleteEvento = async (eventoId: string, userId: string, userRol: string): Promise<IEventoModel | null> => {
