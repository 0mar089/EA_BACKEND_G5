...
const handleMatch = async (userAId: string, userBId: string) => {
  ...
  try {
    ...
    const userAPhotos = await UnimatchPhoto.find({ userId: userAId })
      .sort({ order: 1 })
      .limit(1)
      .lean();
    const userBPhotos = await UnimatchPhoto.find({ userId: userBId })
      .sort({ order: 1 })
      .limit(1)
      .lean();
...
const addPhoto = async (userId: string, imageUrl: string) => {
  ...
  const lastPhoto = await UnimatchPhoto.findOne({ userId }).sort({ order: -1 }).lean();
